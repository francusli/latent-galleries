...
## Standalone Pre-Harness Capability: Curator Charters

Curator charters now exist as a standalone capability before the curation
harness phases. Each curator has an undated JSON fixture in
`data/curators/{curator}.charter.json`, generated by
`npm run generate-charter -- --curator gpt` or
`npm run generate-charter -- --all`.

Future curation phases should load these charters through the shared domain and
storage helpers, preferring the latest dated reviewed charter at
`data/curators/{YYYY-MM-DD}.{curator}.charter.json` when one exists and falling
back to the undated fixture otherwise. Render loaded charters into prompts with
`renderCuratorCharterForPrompt` rather than duplicating charter parsing or using
raw JSON directly in provider prompts.

The charter schema intentionally uses art-facing field names:
`curatorialApproach`, `preferredMedia`, `guidingQuestions`,
